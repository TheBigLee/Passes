...
- **Manual entry** — type in or scan a barcode/QR directly, with a structured form (pick Event/Boarding/Loyalty/Generic, fill in kind-specific fields like location, date/time, or transit mode, and optionally pick a background color) so manually-created passes render just like imported ones.
